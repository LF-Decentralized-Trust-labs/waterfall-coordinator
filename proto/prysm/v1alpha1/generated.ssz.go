...
// Hash: 68436b0f909375a65befa6b28f02cd166b7aa028504bfb5090f2a618a6557da4
package eth
...
// MarshalSSZ ssz marshals the VoluntaryExitList object
func (v *VoluntaryExitList) MarshalSSZ() ([]byte, error) {
	return ssz.MarshalSSZ(v)
}

// MarshalSSZTo ssz marshals the VoluntaryExitList object to a target array
func (v *VoluntaryExitList) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf
	offset := int(4)

	// Offset (0) 'Data'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(v.Data) * 48

	// Field (0) 'Data'
	if len(v.Data) > 16777216 {
		err = ssz.ErrListTooBig
		return
	}
	for ii := 0; ii < len(v.Data); ii++ {
		if dst, err = v.Data[ii].MarshalSSZTo(dst); err != nil {
			return
		}
	}
...
// UnmarshalSSZ ssz unmarshals the VoluntaryExitList object
func (v *VoluntaryExitList) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 4 {
		return ssz.ErrSize
	...
	// Field (0) 'Data'
	{
		...
		v.Data = make([]*VoluntaryExit, num)
		for ii := 0; ii < num; ii++ {
			if v.Data[ii] == nil {
				v.Data[ii] = new(VoluntaryExit)
			}
			if err = v.Data[ii].UnmarshalSSZ(buf[ii*48 : (ii+1)*48]); err != nil {
				return err
			}
		}
	}
...
// SizeSSZ returns the ssz encoded size in bytes for the VoluntaryExitList object
func (v *VoluntaryExitList) SizeSSZ() (size int) {
	size = 4
...
// HashTreeRoot ssz hashes the VoluntaryExitList object
func (v *VoluntaryExitList) HashTreeRoot() ([32]byte, error) {
	return ssz.HashWithDefaultHasher(v)
}

// HashTreeRootWith ssz hashes the VoluntaryExitList object with a hasher
func (v *VoluntaryExitList) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()

	// Field (0) 'Data'
	{
		subIndx := hh.Index()
		num := uint64(len(v.Data))
		if num > 16777216 {
			err = ssz.ErrIncorrectListSize
			return
		}
...
// MarshalSSZ ssz marshals the WithdrawalList object
func (w *WithdrawalList) MarshalSSZ() ([]byte, error) {
	return ssz.MarshalSSZ(w)
}

// MarshalSSZTo ssz marshals the WithdrawalList object to a target array
func (w *WithdrawalList) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf
	offset := int(4)

	// Offset (0) 'Data'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(w.Data) * 104

	// Field (0) 'Data'
	if len(w.Data) > 16777216 {
		err = ssz.ErrListTooBig
		return
	}
	for ii := 0; ii < len(w.Data); ii++ {
		if dst, err = w.Data[ii].MarshalSSZTo(dst); err != nil {
			return
		}
	}
...
// UnmarshalSSZ ssz unmarshals the WithdrawalList object
func (w *WithdrawalList) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 4 {
		return ssz.ErrSize
	...
	{
		buf = tail[o0:]
		num, err := ssz.DivideInt2(len(buf), 104, 16777216)
		if err != nil {
			return err
		}
		w.Data = make([]*Withdrawal, num)
		for ii := 0; ii < num; ii++ {
			if w.Data[ii] == nil {
				w.Data[ii] = new(Withdrawal)
			}
			if err = w.Data[ii].UnmarshalSSZ(buf[ii*104 : (ii+1)*104]); err != nil {
				return err
			}
		}
	}
...
// SizeSSZ returns the ssz encoded size in bytes for the WithdrawalList object
func (w *WithdrawalList) SizeSSZ() (size int) {
	size = 4
...
// HashTreeRoot ssz hashes the WithdrawalList object
func (w *WithdrawalList) HashTreeRoot() ([32]byte, error) {
	return ssz.HashWithDefaultHasher(w)
}

// HashTreeRootWith ssz hashes the WithdrawalList object with a hasher
func (w *WithdrawalList) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()

	// Field (0) 'Data'
	{
		subIndx := hh.Index()
		num := uint64(len(w.Data))
		if num > 16777216 {
			err = ssz.ErrIncorrectListSize
			return
		}
		for _, elem := range w.Data {
			if err = elem.HashTreeRootWith(hh); err != nil {
				return
			}
		}
